...
function createRoughElement(id, x1, y1, x2, y2, type){
  switch (type){
    ...
    default:
      throw new Error(`Tool not selected ${type}`);
  }
}
...
const drawElement = (roughCanvas, context, element) => {
  switch (element.type) {
    ...
    case "text":
      ...
      break;
    default:
      throw new Error(`tool not selected ${element.type}`);
  }
}

const adjusmentRequired = type => ['line', 'rectangle'].includes(type);
...
function App() {
  ...
  const updateElement = (id, x1, y1, x2, y2, type, options) => {
    ...
    switch (type) {
      case "line":
      case "rectangle":
        elementsCopy[id] = createRoughElement(id, x1, y1, x2, y2, type);
        break;
      case "pencil":
      ...
      case "text":
        ...
        const textHeight = 24;
        elementsCopy[id] = {
          ...createRoughElement(id, x1, y1, x1 + textWidth, y1 + textHeight, type),
          text: options.text,
        };
        break;
      default:
  ...
  const handleMouseUp = (event) => {
    ...
    if (selectedElement){
      ...
      if ((action === "drawing" || action === "resizing") && adjusmentRequired(type)){
